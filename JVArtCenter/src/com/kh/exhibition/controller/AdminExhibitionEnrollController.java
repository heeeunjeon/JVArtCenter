...
/**
 * Servlet implementation class AdminExhibitionEnrollController
 */
@WebServlet("/admin_enroll.ex")
public class AdminExhibitionEnrollController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminExhibitionEnrollController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(ServletFileUpload.isMultipartContent(request)) {
            ...
            System.out.println("AdminExhibitionEnrollController : " + ex);
            
            int result = new ExhibitionService().insertExhibition(ex);
            
            if(result > 0) {
                request.getSession().setAttribute("alertMsg", "전시 추가에 성공했습니다.");
                response.sendRedirect(request.getContextPath() + "/admin_list.ex?currentPage=1");
            } else {

                request.setAttribute("errorMsg", "전시 추가에 실패했습니다.");
            }
        }
	}
